...
class MessageViewTestCase(TestCase):
    ...
    def test_user_show_with_likes(self):
        ...
        with self.client as c:
            ...
            self.assertIn("@testuser", str(resp.data))
            # use BeautifulSoup to search HTML for li elements. 
            soup = BeautifulSoup(str(resp.data), 'html.parser')
            # should be 4 li elements with class stat: Messages, Following, Followers, Likes
    ...
    def test_remove_like(self):
        ...
        # m user id is 778, testuser id is 8989
        self.assertNotEqual(m.user_id, self.testuser_id)
        # select the single message that is liked by "testuser" and is a message by "user"
        l = Likes.query.filter(
            Likes.user_id==self.testuser_id and Likes.message_id==m.id
        ).one()
        # Ensure the like is found
        self.assertIsNotNone(l)

        with self.client as c:
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.testuser_id
            # post again to like route to unlike the warble
    ...
    def test_unauthenticated_like(self):
        ...
        with self.client as c:
            # this test is missing sess[CURR_USER_KEY] and is testing
            #   what would happen if no user is logged in 
            resp = c.post(f"/users/add_like/{m.id}", follow_redirects=True)
            self.assertEqual(resp.status_code, 200)
...
# to do:
# replace 'testuser' and 'user' names to be more coherent such as testuser1 testuser2 testuser3 etc